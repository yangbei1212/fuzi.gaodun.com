  ...
  try {
    const response = await axios.post(DOUBAO_CONFIG.API_URL, {
      ...
      messages: [
        {
          role: 'system',
          content: '你是xiaoFu，一个专业的英语单词学习助手。你的主要任务是帮助用户生成个性化的单词卡。当用户上传图片并输入单词时，你需要：1. 分析图片中的人物或动物，2. 结合单词含义设计相关动作和场景，3. 提供单词的详细信息（词性、中文释义、发音等），4. 用友好、有趣的语气回复，就像xiaoFu一样活泼可爱。'
        },
        {
          role: 'user',
          content: userMessage
